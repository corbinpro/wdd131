const recipes = [
	...
	},
	{
		...
		recipeIngredient: [
			"For the Cake Base:",
			"180 g (1 1/2 cups/ 6.3 oz) plain flour",
			"2 medium eggs",
			"100 g (3 1/2 oz) butter soft",
			"2 teaspoons vanilla sugar",
			"60 g (about 1/3 cup/ 2.1 oz) sugar",
			"2 1/2 teaspoons baking powder",
			"For the Vanilla Cream:",
			"250 ml (1 cup/ 8 1/2 fl. oz) milk",
			"40 (1/3 cup/ 1.4 oz) corn flour cornstarch",
			"2 tablespoons sugar",
			...
			"175 g (about 3/4 cup/ 6.2 oz) butter soft",
...
function renderRecipes(recipeArray) {
	if (recipeArray.length === 0) {
		recipeList.innerHTML = '<p class="no-results">No recipes found. Try a different search.</p>';
		return;
	}

	const html = recipeArray.map(recipeTemplate).join("");
	recipeList.innerHTML = html;
}

function recipeMatchesSearch(recipe, query) {
	const keywords = query.toLowerCase().split(/\s+/).filter(Boolean);
	const searchSource = `${recipe.name} ${recipe.description} ${recipe.tags.join(" ")}`.toLowerCase();
	return keywords.every((word) => searchSource.includes(word));
}

function filterAndSortRecipes(query) {
	return recipes
...
function handleSearch(event) {
	...
	const filteredRecipes = filterAndSortRecipes(query);
